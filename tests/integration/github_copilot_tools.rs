...
/// Reproduces the VS Code Copilot race where PostToolUse fires before the file
/// edit is visible on disk, then the VS Code extension emits KnownHuman for the
/// same final content. The AI checkpoint must wait for the edit and claim it.
#[test]
fn test_vscode_replace_string_waits_for_delayed_file_write_before_known_human_save() {
    let repo = TestRepo::new();

    let file_path = repo.path().join("hello-world-test.md");
...
";
    ...
    let session_id = "race-0ae773c0-f1c2-4904-bd18-fb1046ff61cd";
    let tool_use_id = "call_racePostToolUse__vscode-1775585312869";

    let pre_hook_input = json!({
        "timestamp": "2026-07-07T07:58:09.640Z",
        "hook_event_name": "PreToolUse",
        "session_id": session_id,
        "transcript_path": fake_copilot_transcript_path(&repo),
        "tool_name": "replace_string_in_file",
    ...
    let delayed_file_path = file_path.clone();
    let delayed_content = final_content.to_string();
    let writer = std::thread::spawn(move || {
        std::thread::sleep(std::time::Duration::from_millis(750));
        std::fs::write(delayed_file_path, delayed_content).unwrap();
    });

    let post_hook_input = json!({
        "timestamp": "2026-07-07T07:58:10.518Z",
        "hook_event_name": "PostToolUse",
        "session_id": session_id,
        "transcript_path": fake_copilot_transcript_path(&repo),
        "tool_name": "replace_string_in_file",
    ...
    repo.git_ai(&["checkpoint", "mock_known_human", "hello-world-test.md"])
        .unwrap();

    repo.stage_all_and_commit("Apply delayed Copilot edit").unwrap();
    repo.sync_daemon();

    let mut file = repo.filename("hello-world-test.md");
    file.assert_lines_and_blame(crate::lines![
        "# Hello World".human(),
        "Lorem ipsum dolor sit amet, consectetur adipiscing elit.".ai(),
